...
export default function CourseDetailScreen() {
  ...
  useEffect(() => {
    const checkIfCourseIsPurchased = async () => {
      const existingCartData = await AsyncStorage.getItem("cart");
      const cartData = existingCartData ? JSON.parse(existingCartData) : [];
      const isInCart = cartData.some((item: any) => item._id === courseData._id);

      if (isInCart) {
        setCheckPurchased(true);
      } else if (user?.courses?.find((i: any) => i._id === courseData._id)) {
        setCheckPurchased(true);
      } else {
  ...
  const handleAddToCart = async () => {
    ...
    if (!itemExists) {
      ...
      setCheckPurchased(true); // Update state after adding to cart
    }
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {activeButton === "Reviews" && (
              <View style={{ marginHorizontal: 16, marginVertical: 25 }}>
                ...
                {/* <Text>{courseData.user.name}</Text> */}
